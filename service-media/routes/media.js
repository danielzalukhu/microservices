...
router.get('/', async(req, res) => {
  ...
  // Fungsi mappedMedia ini supaya wwaktu ngereturn data imagenya sudah berupa url 
  // yang bisa lagnsung di akses
...
router.post('/', (req, res) => {
  ...
  // Cek dulu jika gambar yg di upload bukan berupa base64
  if(!isBase64( image, { mimeRequired: true }) ) {
    return res.status(400).json({ status: 'error', message: 'invalid base64' })
  }
  // Abis di cek/validasi gambarnya base 64 apa bukan, kalau iya maka jalanin fungsi ini
  // untuk insert ke database
  base64Img.img(image, './public/images', Date.now(), async (err, filepath) => {    
    if(err){
      return res.status(400).json({ status: 'error', message: err.message });
    }
    // Trus buat sebuah penamaan dari file gambar yang diinsert ambil
    const filename = filepath.split("\\").pop().split("/").pop();
    // Jika tidak error maka simpan filepath dari gambar tadi ke dalam database panggil model Media yang 
    // mendefinisikan table media
    const media = await Media.create({ image: `images/${filename}` });
    // Jika berhasil kirim respon 
    return res.json({
      status: 'success',
      data: {
        id: media.id,
